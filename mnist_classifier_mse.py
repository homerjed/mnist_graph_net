...
class LinearResNet(hk.Module):
  def __init__(
    ...
    self.middle_sizes = hidden_sizes 
  ...
  def __call__(self, x):
    ...
    z = x
    for a, b in zip(
      self.hidden_sizes, self.middle_sizes):
      h = self.activation(z)
      h = hk.Linear(b)(h)
      h = self.activation(h)
      h = hk.Linear(a, w_init=jnp.zeros)(h)
      z = z + h 
...
def update_global_fn(feats: Array) -> Array:
  ...
  net = hk.Sequential([
    ...
    LinearResNet(
      [F, F, F],
      activate_final=True),
    hk.Linear(1, with_bias=False)]) # output MNIST classes
  return net(feats)
...
def compute_loss(
  ...
  loss = jnp.square(jnp.subtract(preds * mask[:, None], targets)).mean()

  # Accuracy taking into account the mask.
  accuracy = jnp.sum(
    (jnp.argmax(pred_graph.globals, axis=1) == label) * mask) / jnp.sum(mask)
  return loss, (accuracy, preds * mask[:, None])
...
def train(
  ...
  for _ in range(num_train_steps):
    ...
    idx = int(jr.randint(
        key_idx, shape=(1,), minval=0, maxval=len(dataset)))
    ...
    if idx % 100 == 0:
      print(f'step: {idx:06d}, {label[0]}={pred} loss: {loss:.4f}, acc: {100. * acc:.4f}')
    losses.append(loss)
    accs.append(acc)
...
if __name__ == "__main__":
  ...
  params, losses, accs = train(dataset, num_train_steps=1_000)
